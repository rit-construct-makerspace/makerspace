...
export default function MaintenanceLogEntry({ logItem, allTags, isResolution = false }: { logItem: MaintenanceLogItem, isResolution: boolean, allTags: MaintenanceTag[] }) {
  ...
  const [deleteLog] = useMutation(!isResolution ? DELETE_MAINTENANCE_LOG : DELETE_RESOLUTION_LOG, {
    variables: {id: logItem.id}, 
    refetchQueries: [{ query: isResolution ? GET_RESOLUTION_LOGS : GET_MAINTENANCE_LOGS, variables: {equipmentID: logItem.equipment?.id} }] });
  ...
  const handleRemoveTagClick = (id: number) => {
    removeTag({variables: {logId: logItem.id, tagId: id, logType: (isResolution ? "resolution" : "maintenance")}});
  }

  const handleAddTagClick = (id: number) => {
    addTag({variables: {logId: logItem.id, tagId: id, logType: (isResolution ? "resolution" : "maintenance")}});
