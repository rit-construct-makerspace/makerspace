    ...
    >
        <Grid container spacing={3} mt={2}>
          {getEquipmentsResult.data?.equipments?.filter((m: Equipment) =>
              ...
              <Grid key={e.id} item>
                <EquipmentCard id={e.id??0} name={e.name} to={url + e.id} imageUrl={(e.imageUrl == undefined || e.imageUrl == null ? process.env.PUBLIC_URL + "/shed_acronym_vert.jpg" : "" + process.env.CDN_URL + process.env.CDN_EQUIPMENT_DIR + "/" + e.imageUrl)} />
              </Grid>
          ))}
        </Grid>
    </RequestWrapper>
