...
export async function getNumRoomSwipesToday(): Promise<number | undefined> {
    ...
    endOfDay.setDate(endOfDay.getDate()+1);
    //Length is Best option right now because for some reason knex does not do count correctly
    return (await knex("RoomSwipes").whereRaw(`("dateTime" at time zone 'UTC') BETWEEN TIMESTAMP '${startOfDay.toISOString().replace("T", " ").replace("Z", "")}' AND TIMESTAMP '${endOfDay.toISOString().replace("T", " ").replace("Z", "")}-04'`)).length;
}
