  ...
  app.put("/api/status", async function (req, res) {
    try {
      ...
      if (req.body.Source == "Card Removed") {
        ...
        await setLatestEquipmentSessionLength(parseInt(reader.machineType), req.body.Time, req.body.Machine);
        if (user != undefined) {
          await createLog(`{user} signed out of {machine} - {equipment} (Session: ${new Date(Number(req.body.Time) * 1000).toISOString().substring(11, 16)} sec)`, "status", { id: user.id, label: getUsersFullName(user) }, { id: reader.id, label: req.body.Machine ?? "undefined" }, { id: equipment.id, label: equipment.name });
        }
      }
